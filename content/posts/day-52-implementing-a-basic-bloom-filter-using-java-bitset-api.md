...
* Deleting elements from filter is not possible. Because, if we delete a single element by clearing bits at indices generated by k hash functions, it might cause deletion of few other elements. Example – if we delete “Mohibul” (in given example) by clearing bit at 1, 5 and 2, we might end up deleting “Mary” also if its present Because bit at index 4 becomes 0 and bloom filter claims that “nerd” is not present.
